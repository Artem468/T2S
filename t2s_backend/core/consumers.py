...
class ChatExecutorConsumer(AsyncWebsocketConsumer):
    ...
    async def _ensure_room(self, chat_id: int) -> None:
        """Socket should listen to target chat room even if chat_id changes on same WS."""
    ...
    async def chat_retry(self, event):
        await self.send(
            text_data=json.dumps(
                {
                    "type": "retry",
                    "text": event.get("text") or "Повторная попытка генерации SQL",
